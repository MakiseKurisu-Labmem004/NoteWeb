import { ssrRenderAttrs, ssrRenderSlot, ssrInterpolate, ssrRenderAttr, ssrRenderList, ssrRenderComponent, ssrRenderVNode, ssrRenderClass, ssrIncludeBooleanAttr, ssrLooseContain, ssrLooseEqual, renderToString } from "vue/server-renderer";
...
const siteData = deserializeFunctions(JSON.parse('{"lang":"zh-CN","dir":"ltr","title":"纸间 · Paper Notes","description":"阅读、思考、连接。用 Markdown 持续积累论文阅读笔记。","base":"/NoteWeb/","head":[],"router":{"prefetchLinks":true},"appearance":true,"themeConfig":{"siteTitle":"纸间 / Paper Notes","nav":[{"text":"论文书架","link":"/"},{"text":"写作指南","link":"/guide"},{"text":"关于","link":"/about"}],"search":{"provider":"local","options":{"miniSearch":{"options":{"tokenize":"_vp-fn_(text) => Array.from(new Intl.Segmenter(\\"zh-CN\\", { granularity: \\"word\\" }).segment(text)).filter((part) => part.isWordLike).map((part) => part.segment)"}},"locales":{"root":{"translations":{"button":{"buttonText":"搜索笔记","buttonAriaLabel":"搜索笔记"},"modal":{"displayDetails":"显示详细内容","resetButtonTitle":"清除搜索","backButtonTitle":"关闭搜索","noResultsText":"没有找到相关笔记","footer":{"selectText":"选择","navigateText":"切换","closeText":"关闭"}}}}}}},"outline":{"level":[2,3],"label":"本页目录"},"docFooter":{"prev":"上一篇","next":"下一篇"},"lastUpdated":{"text":"最近更新","formatOptions":{"dateStyle":"medium"}},"returnToTopLabel":"回到顶部","darkModeSwitchLabel":"外观","sidebarMenuLabel":"菜单","footer":{"message":"阅读是起点，理解在字里行间。","copyright":"Paper Notes · 持续生长的研究笔记"}},"locales":{},"scrollOffset":134,"cleanUrls":false}'));
...
const _sfc_main$u = /* @__PURE__ */ defineComponent({
  ...
  __ssrInlineRender: true,
  setup(__props) {
    const VPLocalSearchBox = defineAsyncComponent(() => import("./VPLocalSearchBox.D2_lM9zW.js"));
...
const data = JSON.parse('[{"url":"/papers/example-method.html","title":"如何拆解一篇机器人学习论文","description":"从任务定义到实验结论，用一份结构化示例练习阅读方法，区分作者的主张与实验支持的证据。","date":"2026-09-05","tags":["机器人学习","阅读方法"],"status":"精读中","venue":"阅读示例 · 方法拆解","example":true},{"url":"/papers/example-experiments.html","title":"让实验记录成为可复用的知识","description":"记录环境、参数、结果和失败原因，把一次复现变成之后能够回溯与比较的研究资料。","date":"2026-09-04","tags":["实验复现","阅读方法"],"status":"已读","venue":"阅读示例 · 实验记录","example":true}]');
const _sfc_main$1 = {
  __name: "PaperLibrary",
  __ssrInlineRender: true,
  setup(__props) {
    ...
    const tags = computed(() => ["全部", ...new Set(data.flatMap((p) => p.tags))]);
    const filtered = computed(() => data.filter(
      (p) => (tag.value === "全部" || p.tags.includes(tag.value)) && (status.value === "全部状态" || p.status === status.value) && `${p.title} ${p.description} ${p.tags.join(" ")} ${p.venue}`.toLowerCase().includes(query.value.trim().toLowerCase())
    ));
    const statuses = computed(() => ["全部状态", ...new Set(data.map((p) => p.status))]);
    return (_ctx, _push, _parent, _attrs) => {
      _push(`<main${ssrRenderAttrs(mergeProps({ class: "library" }, _attrs))}><section class="library-hero"><div><p class="eyebrow">A GROWING RESEARCH JOURNAL</p><h1>读过的论文，<br>留下自己的<span>思考。</span></h1><p class="hero-description">把问题、方法与灵感写下来。<br>在一次次阅读中，连接知识的碎片。</p><a class="guide-link"${ssrRenderAttr("href", unref(withBase)("/guide.html"))}>开始写一篇笔记 <span aria-hidden="true">↗</span></a></div><aside class="journal-card" aria-label="笔记库概览"><span class="journal-label">FIELD NOTES / 研究手记</span><div class="journal-number">${ssrInterpolate(String(unref(data).length).padStart(2, "0"))}<small>篇笔记</small></div><div class="journal-line"></div><p>保持好奇，<br>让每一次阅读都有迹可循。</p><span class="journal-bottom">${ssrInterpolate(tags.value.length - 1)} 个研究主题 · 持续更新中</span></aside></section><section class="shelf" aria-labelledby="shelf-title"><div class="shelf-heading"><div><p class="eyebrow">THE COLLECTION</p><h2 id="shelf-title">论文书架</h2></div><span class="result-count" aria-live="polite">${ssrInterpolate(filtered.value.length)} 篇笔记</span></div><div class="shelf-tools"><label class="search-field"><span aria-hidden="true">⌕</span><input${ssrRenderAttr("value", query.value)} type="search" placeholder="筛选标题、关键词或研究方向…" aria-label="筛选笔记"></label><select aria-label="按阅读状态筛选"><!--[-->`);
      ssrRenderList(statuses.value, (s) => {
        _push(`<option${ssrIncludeBooleanAttr(Array.isArray(status.value) ? ssrLooseContain(status.value, null) : ssrLooseEqual(status.value, null)) ? " selected" : ""}>${ssrInterpolate(s)}</option>`);
      });
      _push(`<!--]--></select></div><div class="tag-list" aria-label="研究主题"><!--[-->`);
      ssrRenderList(tags.value, (t) => {
        _push(`<button class="${ssrRenderClass({ active: tag.value === t })}"${ssrRenderAttr("aria-pressed", tag.value === t)}>${ssrInterpolate(t)}</button>`);
      });
      _push(`<!--]--></div>`);
      if (filtered.value.length) {
        _push(`<div class="paper-grid"><!--[-->`);
        ssrRenderList(filtered.value, (paper) => {
          _push(`<a${ssrRenderAttr("href", unref(withBase)(paper.url))} class="paper-card"><div class="paper-top"><span>${ssrInterpolate(paper.venue || "READING NOTE")}</span><span class="status">${ssrInterpolate(paper.status)}</span></div><h3>${ssrInterpolate(paper.title)}</h3><p class="paper-description">${ssrInterpolate(paper.description)}</p><div class="paper-tags"><!--[-->`);
          ssrRenderList(paper.tags, (t) => {
            _push(`<span>${ssrInterpolate(t)}</span>`);
          });
          _push(`<!--]-->`);
          if (paper.example) {
            _push(`<span>示例笔记</span>`);
          } else {
            _push(`<!---->`);
          }
          _push(`</div><div class="paper-bottom"><time${ssrRenderAttr("datetime", paper.date)}>${ssrInterpolate(paper.date)}</time><span aria-hidden="true">阅读笔记 ↗</span></div></a>`);
        });
        _push(`<!--]--></div>`);
      } else {
        _push(`<div class="empty-state"><h3>暂时没有匹配的笔记</h3><p>试试其他关键词，或清除筛选条件。</p><button>显示全部笔记</button></div>`);
      }
      _push(`</section><div class="library-footnote"><span>01 — 阅读 · 02 — 记录 · 03 — 连接</span><span>以 Markdown 书写，以思考积累。</span></div></main>`);
    };
  }
};
...
_sfc_main$1.setup = (props, ctx) => {
  const ssrContext = useSSRContext();
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add(".vitepress/theme/PaperLibrary.vue");
...
const _sfc_main = {
  ...
  __ssrInlineRender: true,
  setup(__props) {
    const { frontmatter: f } = useData$1();
    return (_ctx, _push, _parent, _attrs) => {
      _push(`<div${ssrRenderAttrs(mergeProps({ class: "reading-meta" }, _attrs))}><a${ssrRenderAttr("href", unref(withBase)("/"))}>← 返回论文书架</a><p>${ssrInterpolate(unref(f).date)} · ${ssrInterpolate(unref(f).status)}`);
      if (unref(f).venue) {
        _push(`<!--[--> · ${ssrInterpolate(unref(f).venue)}<!--]-->`);
      } else {
        _push(`<!---->`);
      }
      _push(`</p>`);
      if (unref(f).authors) {
        _push(`<p><strong>作者：</strong>${ssrInterpolate(unref(f).authors)}</p>`);
      } else {
        _push(`<!---->`);
      }
      _push(`<div class="paper-tags"><!--[-->`);
      ssrRenderList(unref(f).tags, (tag) => {
        _push(`<span>${ssrInterpolate(tag)}</span>`);
      });
      _push(`<!--]--></div>`);
      if (unref(f).paper || unref(f).code) {
        _push(`<p>`);
        if (unref(f).paper) {
          _push(`<a${ssrRenderAttr("href", unref(f).paper)} target="_blank" rel="noopener noreferrer">论文原文 ↗</a>`);
        } else {
          _push(`<!---->`);
        }
        _push(` `);
        if (unref(f).code) {
          _push(`<a${ssrRenderAttr("href", unref(f).code)} target="_blank" rel="noopener noreferrer">代码 ↗</a>`);
        } else {
          _push(`<!---->`);
        }
        _push(`</p>`);
...
_sfc_main.setup = (props, ctx) => {
  const ssrContext = useSSRContext();
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add(".vitepress/theme/PaperMeta.vue");
  return _sfc_setup ? _sfc_setup(props, ctx) : void 0;
};
const RawTheme = {
  extends: theme,
  enhanceApp({ app }) {
    app.component("PaperLibrary", _sfc_main$1);
    app.component("PaperMeta", _sfc_main);
  }
};
